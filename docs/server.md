...
Для нескольких подключений на одном компьютере используйте
`agent-sync profile add <name>` и `--profile <name>` в командах ниже.
Для просмотра серверной истории отдельно от местных сессий зарегистрируйте
профиль с `--sessions-dir`. [Выбор и настройка профилей](profiles.md).
