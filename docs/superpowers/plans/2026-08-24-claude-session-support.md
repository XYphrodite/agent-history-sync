...
- [x] `Management/ClaudeSessionCatalogSource.cs` mirroring `GrokSessionCatalogSource`: bounded prefix read (`64 KiB`, 64 records), title per design D7 — newest `ai-title`, else `summary`, else first non-technical user text — `MaximumTitleLength = 80`, duplicates demoted to `CanRead = false`.
- [x] **Corrected after release testing:** the newest `ai-title` of a long session sits past the prefix, so a prefix-only read named a 668 KiB session after its opening turn. A transcript the prefix did not cover now gets a second bounded read of the trailing `64 KiB`, whose newest `ai-title` outranks the prefix; the `<user_query>` wrapper is stripped before the technical-preview check, and the hidden primer turn is skipped, so the last-resort title is a turn the user wrote.
...
- [x] Claude home override alongside the Grok home handling in `SystemCliAdapters`. **Corrected while implementing:** there is no `--grok-home` or `--codex-home` flag to sit beside, and no persisted home setting — agent homes are overridden only by environment variable (`CODEX_HOME`, `GROK_HOME`) plus a constructor parameter used by tests. Claude follows exactly that: `CLAUDE_CONFIG_DIR` and a `claudeHome` parameter. Introducing a `--claude-home` flag would have been the only such flag in the CLI; left for a separate decision about a general `--<agent>-home` option set.
- [x] `status` and `doctor` report the resolved Claude home, session count, and uncertainty.
- [x] `README.md` (English and Russian sections) gains Claude in the agent table and the intro line.
- [x] `docs/operations.md` — a Claude sessions section and the upgrade-before-push gate; `docs/security.md` — only `projects/**/*.jsonl` leaves the machine.
- [x] Version bump to 0.6.0. Release notes are generated by GitHub from commits (`generate_release_notes: true` in the release workflow), so the D4 gate is stated where a user will actually meet it: a callout at the top of `README.md` and its own subsection in `docs/operations.md`.
